...
void tas_sim(int thread=0){
    ...
    for (int j = 0; j<filelist_samp.size(); j++) {
        ...
        t->Draw("eneAll>>TAS(10000,1,10001)","eneDSSD>0&&multi==1","");
        ...
        if( thread <1){
            std::ofstream myFile(Form("tas_new_m1/output_%s.csv",energy.c_str()));
        ...
        std::cout << thread << std::endl;
            std::ofstream myFile(Form("%d/tas_new_m1/output_%s.csv",thread,energy.c_str()));
            for ( int i = 1; i<h1->GetNbinsX()+1; i++){
                myFile << i << "," << h1->GetBinContent(i) << "\n";
            }
            myFile.close();
