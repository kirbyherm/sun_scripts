...
//    t->Draw(Form("ene%s%d>>simSeg_%s_%s%d(12000,1,12001)",side[i].c_str(),j,histname.c_str(),side[i].c_str(),j),"","");
    ...
    t->Draw(Form("ene%s%d>>simSeg_%s_%s%d(%d,%d,%d)",side[i].c_str(),j,histname.c_str(),side[i].c_str(),j,int((ss_upper_limit-ss_lower_limit)/bin_width),ss_lower_limit,ss_upper_limit),Form("%s&&multi==1", cuts.c_str()),"");
